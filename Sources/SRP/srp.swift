...
public struct SRP<H: HashFunction> {
        ...
        hashSharedSecret: [UInt8]) -> [UInt8]
    {
        ...
        let gBytes = configuration.shouldPadG ? (SRP<H>.pad(configuration.g.bytes, to: configuration.sizeN)) : configuration.g.bytes
        let N_xor_g = [UInt8](H.hash(data: configuration.N.bytes)) ^ [UInt8](H.hash(data: gBytes))
